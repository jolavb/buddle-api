...
class StatesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create state" do
    assert_difference('State.count') do
      post states_url, params: { state: { abr: @state.abr, desc: @state.desc, name: @state.name, status: @state.status } }, as: :json
    end
  ...
  test "should update state" do
    patch state_url(@state), params: { state: { abr: @state.abr, desc: @state.desc, name: @state.name, status: @state.status } }, as: :json
    assert_response 200
  end
